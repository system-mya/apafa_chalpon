...
iniciar_sesion_app(user, res) {
	connection.acquire((err, con) => {
		if(err){
			res.send({status: 0, message: err.sqlMessage});
		}else{;
		var query = "CALL pa_iniciar_sesion_aplicativo_movil('"+[user.datobusqueda] +"')";
		//res.send("CALL INICIAR_SESION('"+ [user.username] +"','"+ [user.password] +"','"+ hoyFecha()+ "')");
		con.query(query,(err, result) => {
			con.release();
			if(err){
				...
				if (result[0].length == 0) {
					res.send({status: 2, message: 'DOC ALUMNO NO EXISTE'});
				} else {
					res.send({status: 1, message: 'LOGIN EXITOSO',data:result[0]});
				}
			}
...
update_anhio_xcriterio(anhio, res) {
	connection.acquire((err, con) => {
		if(err){
		  ...
		  if([anhio.datobusqueda]=='reiniciar'){
            con.query('CALL pa_verificar_si_anhio_iniciado()', (err, result) => {
				if(err){
					res.send({status: 0, message: err.sqlMessage});
				}else{
					if (result[0].length == 0) {
								...
								if (result.affectedRows == 0) {
									...
									con.query('CALL pa_verificar_si_anhio_iniciado()',(err, result) => {
										...
										if(err){
											...
											if (result.affectedRows == 0) {
												res.send({status: 3, message: 'AÑO LECTIVO NO INICIADO'});
											} else {												
												res.send({status: 1, message: 'AÑO LECTIVO INICIADO',data:result[0]});
											}
										}
									});
								}
						...
						res.send({status: 2, message: 'DEBE CERRAR AÑO INICIADO'});
					}
				}
