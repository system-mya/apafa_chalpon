...
class Apafa {
    ...
    nvo_alumno(alumno, res) {
        connection.acquire((err, con) => {
            ...
            if ([alumno.tdoc_padre]==''){
                var tdoc_padre="NULL";
            }else{
                tdoc_padre="'"+[alumno.tdoc_padre]+"'";
            }

            if ([alumno.doc_padre]==''){
                var doc_padre="NULL";
            }else{
                doc_padre="'"+[alumno.doc_padre]+"'";
            }

            if ([alumno.tdoc_madre]==''){
                var tdoc_madre="NULL";
            }else{
                tdoc_madre="'"+[alumno.tdoc_madre]+"'";
            }

            if ([alumno.doc_madre]==''){
                var doc_madre="NULL";
            }else{
                doc_madre="'"+[alumno.doc_madre]+"'";
            }
            ...
            con.query(query_doc,(err, result) => {
                ...
                if(err){
                    ...
                    if (result[0].length == 0) {
                        ...
                        + "," + tdoc_padre + "," + doc_padre + "," + tdoc_madre + "," + doc_madre + ")";
...
update_alumno(alumno, res) {
    ...
    listar_matriculados(res) {
        connection.acquire((err, con) => {
            ...
            con.query("CALL pa_listar_padron_matriculados_xanhio()", (err, result) => {
                con.release();
                if(err){
                    res.send({status: 0, message: err.sqlMessage});
                }else{
    ...
    obtener_total_deuda_xanhio(alumno, res) {
        connection.acquire((err, con) => {
            if(err){
                res.send({status: 0, message: err.sqlMessage});
            }else{
            var array = alumno.datobusqueda.split("-");
            var query = "CALL pa_obtener_total_deuda("+ array[0] +","+array[1]+")"; 
            /* res.send("CALL pa_obtener_usuario("+ [user.idbusqueda] +")");  */
            con.query(query,(err, result) => {
                con.release();
                if(err){
                    ...
                    if (result[0].length == 0) {
                        res.send({status: 2, message: 'Sin Deuda Registrada'});
                    } else {
                        res.send({status: 1, message: 'Datos Alumno',data:result[0]});
                    }
                }
