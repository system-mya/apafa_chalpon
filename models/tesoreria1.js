...
class Tesoreria {
    ...
    listar_recibos_apoderado(recibo, res) {
        connection.acquire((err, con) => {
            if(err){
                res.send({status: 0, message: err.sqlMessage});
            }else{
            var query = "CALL pa_listar_recibos_apoderados("+[recibo.idbusqueda]+")"; 
            /* res.send("CALL pa_obtener_usuario("+ [user.idbusqueda] +")");  */
            con.query(query,(err, result) => {
                con.release();
                if(err){
    ...
    listar_reuniones_apoderado(apoderado, res) {
        connection.acquire((err, con) => {
            if(err){
                res.send({status: 0, message: err.sqlMessage});
            }else{
            var query = "CALL pa_listar_reuniones_apoderado("+[apoderado.idbusqueda]+")"; 
            /* res.send("CALL pa_obtener_usuario("+ [user.idbusqueda] +")");  */
            con.query(query,(err, result) => {
                con.release();
                if(err){
