...
import {clsPadron_Matricula,clsGrados,clsSecciones,clsTipo_Relacion,clsBusqueda,clsLibro,clsLibro_Matricula} from '../../app.datos';
...
export class MatriculaComponent {
  ...
  public matricula : clsPadron_Matricula = {};
...
DataGrado : clsGrados;
 ...
 Listar_Secciones_xGrado(id){
    ...
    .then(data => {
      if(data.status==1){
        ...
        if(id==0){
          ...
          this.matricula.id_tipo_relacion=0;
        }
       }
    } )
    .catch(err => console.log(err))
  ...
  onSubmit(form:clsPadron_Matricula){    
  ...
  btnEliminar_Matricula(id:number) {    
    swal({
      title: '¿Esta seguro que desea eliminar matricula?',
      type: 'question',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
