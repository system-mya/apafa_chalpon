...
applyFilter(filterValue: string) {
  ...
  RegOtroIngreso(form: clsMovimiento) {
  ...
  ImprimirPDF(dato){
    console.log(dato);
      if(dato.tipo.charAt(0)=='R'){
           this.VerPDFRecibo(dato.id_ingreso,dato.doc_ingreso,dato.freg_ingreso,dato);
      }else{
         this.VerPDFIngreso(dato.id_ingreso);
      }
  }
  public detalle_recibo;
  public VerPDFRecibo(id_ingreso,num,fecha,dator)
  {
  ...
  const doc = new jspdf({orientation: 'portrait',unit: 'mm',format: 'A5'});
           ...
           var descripcion;
            ...
            doc.text(25, 70, 'Señor(a): '+ dator.nompago);
            doc.text(25, 80, 'Documento Identidad: '+ dator.docpago);
            doc.text(125, 80, 'Num. contacto: ' + dator.celupago);
            if(dator.desc_recibo==null){
              descripcion = '';
            }else{
              descripcion = dator.desc_recibo;
            }
           var splitTitle = doc.splitTextToSize('Descripción: ' + descripcion, 160);
        ...
        data_recibo => {
          ...
          doc.text(150, doc.autoTable.previous.finalY + 10, ''+suma_total.toFixed(2));
              ...
              this.toastr.success('Archivo PDF Generado', 'Aviso!',{positionClass: 'toast-top-right',timeOut: 1000});
              this.DetallePago.hide();
    ...
    if(data.status==1){    
           ...
           var totalPagesExp = "{total_pages_count_string}";
            ...
            doc.setFontType('bold');
              ...
              this.toastr.success('Recibo Generado', 'Aviso!',{positionClass: 'toast-top-right',timeOut: 1000});
              this.DetallePago.hide();
              this.loadingBar.complete();
              this.spinner.hide();
              this.document.documentElement.scrollTop = 0;
  ...
  btnBuscar_xDoc(dato:string){
    ...
    this.DatoBusqueda.datobusqueda=dato;
      ...
      .then(data => {
        if(data.status==1){
            ...
            this.recibo.docpago = data.data[0].doc_apoderado;
            this.recibo.nompago = data.data[0].apellidos_apoderado + " " + data.data[0].nombres_apoderado;
            this.recibo.celpago = data.data[0].celular_apoderado;
            this.recibo.desc_recibo = null;
            this.datosrecibo = true;
    ...
    public Cambiar_Recibo(dato){
      if(dato==true){
        this.datosrecibo = false;
        this.recibo.docpago = '';
        this.recibo.nompago = '';
        this.recibo.celpago = '';
      }else{
        this.datosrecibo = true;
        this.recibo.docpago = this.recibo.doc_apoderado;
        this.recibo.nompago = this.recibo.datos_apoderado;
        this.recibo.celpago = this.recibo.celular_apoderado;
      }
    ...
    public RegRecibo(form:clsRecibo){
      swal({
        ...
        if (result.value == true) {
          ...
          form.contador=0;
          this.spinner.show();
          this.spinner_mensaje = 'Registrando Recibo';
          ...
          .then(data => {
            if (data.status == 1) {
              setTimeout(() => {
                ...
                if (result.value == true) {   
                  console.log(data);               
                  this.VerPDFRecibo(data.data[1][1],data.data[0],data.data[2],data.data[3]);
                  this.ListarIngresos();
                }else{
                  this.ListarIngresos();
                }
    ...
    public monto_parcial(index,tipo){
      ...
      console.log(Number(this.DataDeuda[index].monto));
      var indice;
      this.recibo.mtotal_recibo=0;
     if(Number(this.DataDeuda[index].monto)<=Number(this.DataDeuda[index].saldo_deuda)){
      console.log('el monto es menor');
      if((Number(this.DataDeuda[index].monto))===Number(0)){
        console.log('el monto es cero');
...
btnDetalle_Ingreso(dato){
  ...
  if(dato.tipo.charAt(0)=='R'){;
        ...
        this.DetApoderado.datos_recibo = dato.nompago;   
        this.DetApoderado.doc_apoderado = dato.docpago;
        this.DetApoderado.celular_apoderado = dato.celupago;
        this.DetApoderado.descripcion = dato.desc_recibo;    
  ...
  btnEliminar_Ingreso(dato) {
    ...
    swal({
      title: '¿Esta seguro que desea eliminar ingreso?',
      type: 'question',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
